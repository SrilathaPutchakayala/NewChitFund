...
public class Login extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		etUserName.setTypeface((Typeface.createFromAsset(getAssets(),"fonts/Roboto-Light.ttf")));
		...
		btnLogin.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// Authenticate user
				if (dataValid()) { new LoginAsyncTask().execute(); }
	...
	/**
	 * This method authenticate member by the given details
		...
		protected void onPostExecute(String result) {
			...
			if(pd.isShowing()){
				...
				if(!checkNet){
					...
					if(result.equalsIgnoreCase("Phone not registered.")) {
						etUserName.setText("");
						etPassword.setText("");
						etUserName.requestFocus(1);
					}else if(result.equalsIgnoreCase("Password does not match, please try again.")) {
						etPassword.setText("");
						etPassword.requestFocus(1);
					}
					
					// Show status message to user
					CommonMethods.showErrorDialog(result, Login.this);
