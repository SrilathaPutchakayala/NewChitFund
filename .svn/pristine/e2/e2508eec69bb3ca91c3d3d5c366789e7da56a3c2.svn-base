...
public class ViewChits extends Activity
{
			...
			public void onItemClick(AdapterView<?> av, View v, int position,
				...
				if(fromScreen!=null)
				{
					if(fromScreen.equals("payment"))
					{
						...
						try
						{
							date = inputFormat1.parse(chit.get_bidDate());
							outPutBidDate = outputFormat1.format(date);
						} catch (ParseException e) {
							e.printStackTrace();
						}

						String ChitStartDate = chit.get_startDate()+"-"+outPutBidDate;
						...
						try {
							...
							if (System.currentTimeMillis() < strDate.getTime()) {
								Toast.makeText(getApplicationContext(), "Chit not started.", Toast.LENGTH_SHORT).show();
								//CommonMethods.showErrorDialog("Chit not started", ViewChits.this);
							}
							else
							{
	...
	public void refresh(View v){
		/*
		Intent refreshActivityIntent = new Intent(this,ViewChits.class);
	...
	class GetAllChits extends AsyncTask<Void, Void, String>
	{
		...
		protected String doInBackground(Void... params) {
			try {
				...
				checkNet = Utils.isInternetAvailable(ViewChits.this);
				...
				if(checkNet){
					...
					try {
						...
						//CommonMethods.showErrorDialog(e.getMessage(), ViewChits.this);
					}
		...
		protected void onPostExecute(String result) {
			...
			if(pd.isShowing())
			{
				if(result.equalsIgnoreCase("success")) {
					...
					//CommonMethods.showErrorDialog("Please check network connection", ViewChits.this);				
				}
				else if(result.equalsIgnoreCase("SERVICE_ERROR")) {
					pd.dismiss();
					Toast.makeText(getApplicationContext(), "Service error,please contact administrator", Toast.LENGTH_LONG).show();
					//CommonMethods.showErrorDialog("Service error,please contact administrator", ViewChits.this);
				}
				else{
